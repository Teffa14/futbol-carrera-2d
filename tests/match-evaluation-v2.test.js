...
const {updatePerception}=await import('../perception-scanning-v1.js');
...
test('decision utility compares pass dribble and shot instead of farming one action type',()=>{const e=engine('ST'),p=e.playerById('user-player'),m=e.playerById('mate'),d=e.playerById('def'),o=e.playerById('opp');p.x=875;p.y=350;p.data.shooting=90;p.data.composure=88;m.x=650;m.y=470;d.x=760;d.y=240;o.x=700;o.y=520;e.ball.x=p.x+15;e.ball.y=p.y;p.facingX=-1;p.facingY=0;updatePerception(e,p,{force:true});p.facingX=1;p.facingY=0;e.tick+=2;const choices=scoreActionCandidates(e,p);assert.ok(choices.some(x=>x.type==='pass'));assert.ok(choices.some(x=>x.type==='dribble'));assert.ok(choices.some(x=>x.type==='shot'));assert.equal(choices[0].type,'shot','a striker in a strong central finishing window should prefer the highest-value shot');});