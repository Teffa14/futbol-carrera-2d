...
test('every role drill executes on the player position it belongs to',()=>{for(const drill of ROLE_DRILLS){assert.equal(trainingFamily(playerFor(drill).position),drill.family,drill.id);const {out}=run(drill);assert.equal(out.engineVersion,TRAINING_RUNTIME_VERSIONS.matchEngine,drill.id);assert.equal(out.reps,2,drill.id);assert.ok(Array.isArray(out.markers)&&out.markers.length===drill.markers.length,`${drill.id}: marcadores`);assert.ok(out.successes>=1,`${drill.id}: terminó ${out.successes}/2; el ejercicio no cumple su objetivo ni una vez`);}});
