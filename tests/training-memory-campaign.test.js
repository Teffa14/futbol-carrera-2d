...
test('prepared up-back-through pattern becomes two coordinated physical pass intentions',()=>{
  const home=lineup('h'),away=lineup('a'),e=new MatchEngine(home,away,{seed:'pattern'});e.restart.active=false;e.rng=()=>0;
  const source=e.players.find(x=>x.team===0&&x.role==='CM'),preferredSet=e.players.find(x=>x.team===0&&x.role==='CAM');source.data.trainingMemory={'third-man':{familiarity:90},'through-ball':{familiarity:90}};Object.assign(source,{x:400,y:350});Object.assign(preferredSet,{x:475,y:335});
  const st=e.players.find(x=>x.team===0&&x.role==='ST');Object.assign(st,{x:620,y:350});for(const o of e.players.filter(x=>x.team===1)){o.x=850;o.y=80+(o.slot%6)*85;}
  Object.assign(e.ball,{x:source.x+source.r+e.ball.r-1,y:source.y,vx:0,vy:0});source.decisionCooldown=0;e.prepareBallAction(source);
  const seq=e.teamSequences?.[0];assert.equal(seq?.type,'up-back-through');const actualSet=e.playerById(seq.setId),actualRunner=e.playerById(seq.runnerId);assert.ok(actualSet&&actualRunner);assert.equal(source.kickIntent?.receiverId,actualSet.id);assert.notEqual(actualSet.id,actualRunner.id);
  Object.assign(e.ball,{x:actualSet.x+actualSet.r+e.ball.r-1,y:actualSet.y});actualSet.decisionCooldown=0;e.registerPhysicalTouch(actualSet,'touch');e.prepareBallAction(actualSet);
  assert.equal(actualSet.kickIntent?.receiverId,actualRunner.id);assert.equal(actualSet.kickIntent?.passKind,'through');assert.ok(e.events.some(x=>String(x.text||'').includes('pase en profundidad')));
});
