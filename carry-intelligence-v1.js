...
function carryStats(p){
  const d=p?.data||p||{};
  return{control:Number(d.ballControl??65),dribbling:Number(d.dribbling??65),pace:Number(d.pace??70)};
}
...
export function carryPlan(engine,p,intentTarget,dt=.016){
  ...
  if(ballSpeed>.15&&movingWithIntent>.05){
    const baseMomentum=clamp(.12+ballSpeed*.04,.12,.34),turnRelease=1-clamp(rawTurn/1.05,0,.94),keepMomentum=baseMomentum*turnRelease;
    faceDesired=unit(rawDesired.x*(1-keepMomentum)+ballDir.x*keepMomentum,rawDesired.y*(1-keepMomentum)+ballDir.y*keepMomentum);
  }
  const facingDir=smoothedCarryDirection(p,faceDesired,dt);

  // Contact geometry follows the requested football direction immediately. Facing is allowed to lag.
  // This lets the player get to the correct side of a free ball before the next physical touch.
  const contactDir=rawDesired,perp={x:-contactDir.y,y:contactDir.x},contact=(p.r||7.25)+(ball.r||4.35)-.45,rel={x:ball.x-p.x,y:ball.y-p.y};
  const forwardGap=dot(rel.x,rel.y,contactDir.x,contactDir.y),lateralGap=dot(rel.x,rel.y,perp.x,perp.y),idealBehind={x:ball.x-contactDir.x*contact,y:ball.y-contactDir.y*contact},behindError=dist(p,idealBehind);
  const lateralTolerance=Math.max(4.0,(p.r||7.25)*.66),aligned=Math.abs(lateralGap)<lateralTolerance&&forwardGap>contact*.42&&behindError<Math.max(6.6,contact*.58);
  ...
  if(!aligned){
    // A carry recovery must close the actual current contact gap. Predicting several frames ahead
    // makes the carrier orbit a moving target and is what caused the observed sideways loss.
    const predictFrames=clamp(ballSpeed*.10,0,.55),future={x:ball.x+(ball.vx||0)*predictFrames,y:ball.y+(ball.vy||0)*predictFrames};
    const brake=rawTurn>.34&&ballSpeed>.20,sideCorrection=clamp(lateralGap*.10,-3.2,3.2);
    moveTarget={x:future.x-contactDir.x*contact-perp.x*sideCorrection,y:future.y-contactDir.y*contact-perp.y*sideCorrection};
    phase=brake?'cut':'recover';
  }else{
    moveTarget={x:ball.x+contactDir.x*stride,y:ball.y+contactDir.y*stride};
    phase='touch';
  }
  const facingTarget={x:ball.x+facingDir.x*(70+controlScale*35),y:ball.y+facingDir.y*(70+controlScale*35)};
  return{moveTarget,facingTarget,dir:facingDir,contactDir,phase,aligned,turnSharpness:rawTurn,forwardGap,lateralGap,behindError,ballSpeed};
}
...
MatchEngine.prototype.movePlayer=function continuousPhysicalCarry(p,target,dt,track){
  ...
  p.carryState={...(p.carryState||{}),dir:plan.dir,contactDir:plan.contactDir,phase:plan.phase,aligned:plan.aligned,turnSharpness:plan.turnSharpness,lastTick:this.tick};
  if(plan.phase==='cut'){
    // A real cut requires the runner to take speed out of his own stride before coming around the ball.
    // The ball is untouched here and continues under normal free-ball damping.
    const control=carryStats(p).control,brake=clamp(.90+(control-50)*.0009,.90,.945);p.vx*=brake;p.vy*=brake;
  }
...
MatchEngine.prototype.dribbleTouchPower=function progressiveCarryTouch(p){
  ...
  if(state?.phase==='cut')return clamp(base*.62,.12,.42);
