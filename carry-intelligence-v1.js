...
function smoothedCarryDirection(p,desired,dt){
  const stats=carryStats(p),previous=p.carryState?.dir&&Number.isFinite(p.carryState.dir.x)?p.carryState.dir:{x:p.facingX||desired.x,y:p.facingY||desired.y};
  const delta=angle(previous,desired),turnRate=2.45+(stats.dribbling*.018)+(stats.control*.014),step=clamp(delta,-turnRate*dt,turnRate*dt);
  return unit(...Object.values(rotate(previous,step)));
}

export function carryPlan(engine,p,intentTarget,dt=.016){
  if(!engine?.ball||!p||!intentTarget)return null;
  const ball=engine.ball,stats=carryStats(p),rawDesired=unit(intentTarget.x-ball.x,intentTarget.y-ball.y),ballSpeed=mag(ball.vx||0,ball.vy||0),ballDir=ballSpeed>.05?unit(ball.vx,ball.vy):rawDesired;
  ...
  if(ballSpeed>.15&&movingWithIntent>.05){
    const keepMomentum=clamp(.18+ballSpeed*.055,.18,.48);
  ...
  const previous=p.carryState?.dir||desired,turnSharpness=Math.abs(angle(previous,rawDesired));
  const controlScale=clamp((stats.control+stats.dribbling-90)/110,.18,.92),stride=clamp(14+stats.pace*.055+ballSpeed*2.4,16,27)*(turnSharpness>.48?.58:turnSharpness>.25?.78:1);
  let moveTarget,phase;
  if(!aligned||behindError>Math.max(7.5,contact*.68)){
...
MatchEngine.prototype.dribbleTouchPower=function progressiveCarryTouch(p){
  ...
  const stats=carryStats(p),speed=mag(p.vx||0,p.vy||0),control=(stats.control+stats.dribbling)/2;
  const continuous=.20+speed*.145+control*.00205;
  const turnFactor=state.turnSharpness>.55?.68:state.turnSharpness>.30?.82:1;
  return clamp(Math.max(base,continuous*turnFactor),.14,.88);
};

export const __carryIntelligenceV1={carryStats,smoothedCarryDirection};
