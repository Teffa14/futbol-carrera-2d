...
export function carryPlan(engine,p,intentTarget,dt=.016){
  ...
  const facingDir=smoothedCarryDirection(p,faceDesired,dt),contactDir=rawDesired,perp={x:-contactDir.y,y:contactDir.x},contact=(p.r||7.25)+(ball.r||4.35)-.45,rel={x:ball.x-p.x,y:ball.y-p.y};
  ...
  if(!aligned){
    const predictFrames=clamp(ballSpeed*.07,0,.38),future={x:ball.x+(ball.vx||0)*predictFrames,y:ball.y+(ball.vy||0)*predictFrames},brake=rawTurn>.34&&ballSpeed>.20,sideCorrection=clamp(lateralGap*.08,-2.5,2.5);
    // Aim the player centre slightly through the collision boundary. resolveBallPlayerCollisions()
    // remains the only mechanism that moves the ball; this just guarantees actual re-contact.
    const penetration=clamp(2.5+ballSpeed*.22,2.5,3.5),recontactRadius=Math.max((p.r||7.25)*.72,contact-penetration);
  ...
  }else{moveTarget={x:ball.x+contactDir.x*stride,y:ball.y+contactDir.y*stride};phase='touch';}
...
MatchEngine.prototype.movePlayer=function continuousPhysicalCarry(p,target,dt,track){
  ...
  const intent=p.dribbleIntent,oldX=intent.targetX,oldY=intent.targetY;intent.targetX=plan.facingTarget.x;intent.targetY=plan.facingTarget.y;p.carryState={...(p.carryState||{}),dir:plan.dir,contactDir:plan.contactDir,phase:plan.phase,aligned:plan.aligned,turnSharpness:plan.turnSharpness,lastTick:this.tick};
