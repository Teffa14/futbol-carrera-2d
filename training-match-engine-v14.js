...
function anchor(e,p,target,dt,damping=.35){if(!p||!target)return;e._moved?.add(p.id);p.vx*=damping;p.vy*=damping;face(e,p,target,dt);}
...
function one(e,dt){
  const q=e.trainingQualityV6,[server]=e.mates,[marker,keeper]=e.defenders;observe(e);if(q.repTerminal)return;
  hold(e,marker,{x:885,y:FIELD.centerY-q.side*118},dt);hold(e,keeper,{x:FIELD.right-27,y:FIELD.centerY-q.side*14},dt);const target=goalTarget(q.side);
  ...
  if(q.customStage==='profile'){q.phase='Terminar de perfilar';e.player.dribbleIntent=null;e.player.kickIntent=null;anchor(e,e.player,target,dt,.3);if(facingDot(e.player,target)>.992&&age(e)>.10)stage(e,'serve');return;}
  if(q.customStage==='serve'){q.phase='Sale el pase raso';anchor(e,e.player,target,dt,.3);if(facingDot(e.player,target)<.985)return;if(physicalKick(e,server,{x:e.player.x,y:e.player.y},2.25,'service',dt,{direct:true})){q.serviceTick=e.tick;q.expectedDirectReceiverId=e.player.id;e.pending(server,e.player,'service');e.armKick(e.player,target,6.1,'shot',{trainingKind:'shot'});stage(e,'strike');}return;}
  if(q.customStage==='strike'){q.phase='Rematar de primera';anchor(e,e.player,target,dt,.4);if(!e.player.kickIntent)e.armKick(e.player,target,6.1,'shot',{trainingKind:'shot'});markShot(e);if(q.shotTick!=null){stage(e,'flight');return;}if(age(e)>3.0)terminal(e,false,'NO LLEGÓ','missed-service');return;}
  q.phase='Resolver remate';const out=shotOutcome(e,keeper);if(out)terminal(e,out.success,out.text,out.reason);
}
...
function setupRun(e,rep){
  ...
  Object.assign(e.trainingQualityV6,{side,lineX:748,wait:{x:690,y:FIELD.centerY+side*100},runLane:{x:790,y:FIELD.centerY+side*78},lead:{x:850,y:FIELD.centerY+side*12},possessionId:passer.id});stage(e,'wait');e.repOrigin={px:e.player.x,py:e.player.y,bx:e.ball.x,by:e.ball.y};
}
function run(e,dt){
  ...
  if(q.customStage==='lane'){q.phase='Acelerar por fuera de la trayectoria';hold(e,e.player,q.runLane,dt);face(e,e.player,q.lead,dt);if(e.ball.x>q.lineX-10)stage(e,'attack');else if(age(e)>3.2)terminal(e,false,'PASE CORTO','through-short');return;}
  ...
  if(receivedAfter(e,e.player,q.serviceTick)&&(e.ball.x>q.lineX-4||e.player.x>q.lineX+10)){m.throughReceptions=Math.max(1,m.throughReceptions||0);terminal(e,true,'PASE FILTRADO RECIBIDO','through-received');return;}
...
function setupWall(e,rep){
  ...
  e.resetActor(mate,570,FIELD.centerY-side*10,'CAM');e.resetActor(e.player,680,FIELD.centerY+side*48,'ST');e.resetActor(d1,790,220,'CB');e.resetActor(d2,790,480,'CB');e.resetActor(keeper,FIELD.right-27,FIELD.centerY,'GK');
  ...
  Object.assign(e.trainingQualityV6,{side,support:{x:645,y:FIELD.centerY+side*20},mateSpot:{x:600,y:FIELD.centerY-side*8},breakPoint:{x:735,y:FIELD.centerY+side*64},runLane:{x:795,y:FIELD.centerY+side*66},lead:{x:855,y:FIELD.centerY+side*10},lineX:775,possessionId:mate.id,wallTick:-1,returnTick:-1});stage(e,'check');e.repOrigin={px:e.player.x,py:e.player.y,bx:e.ball.x,by:e.ball.y};
}
function wall(e,dt){
  ...
  if(q.customStage==='service'){q.phase='Pase al pie';hold(e,e.player,q.support,dt);face(e,e.player,mate,dt);hold(e,mate,q.mateSpot,dt);if(physicalKick(e,mate,{x:q.support.x+2,y:q.support.y},1.9,'service',dt,{direct:true})){q.serviceTick=e.tick;q.expectedDirectReceiverId=e.player.id;e.pending(mate,e.player,'service');e.armKick(e.player,q.mateSpot,2.0,'pass',{trainingKind:'wall'});stage(e,'layoff');}return;}
  if(q.customStage==='layoff'){
    q.phase='Descargar de primera';anchor(e,e.player,q.mateSpot,dt,.45);hold(e,mate,q.mateSpot,dt);face(e,mate,q.lead,dt);if(!e.player.kickIntent)e.armKick(e.player,q.mateSpot,2.0,'pass',{trainingKind:'wall'});
    const k=e.lastTrainingKick;if(k?.rep===e.rep&&k.by===e.player.id&&k.kind==='wall'){q.wallTick=k.tick;e.ball.trainingDirectPass=true;q.expectedDirectReceiverId=mate.id;e.pending(e.player,mate,'wall');m.wallBeats=(m.wallBeats||0)+1;stage(e,'return-ready');return;}if(age(e)>3)terminal(e,false,'NO DESCARGÓ','layoff-missed');return;
  }
  if(q.customStage==='return-ready'){
    q.phase='Arrancar tras la descarga';hold(e,e.player,q.breakPoint,dt);face(e,e.player,q.runLane,dt);const intercept=e.projectedIntercept(mate);hold(e,mate,{x:clamp(intercept.x,570,630),y:clamp(intercept.y,300,400)},dt);face(e,mate,q.lead,dt);
    if(receivedAfter(e,mate,q.wallTick)){e.ball.trainingDirectPass=false;stage(e,'return');return;}if(age(e)>3)terminal(e,false,'DESCARGA PERDIDA','wall-missed');return;
  }
  if(q.customStage==='return'){q.phase='Devolver al espacio';hold(e,e.player,q.breakPoint,dt);face(e,e.player,q.runLane,dt);face(e,mate,q.lead,dt);if(physicalKick(e,mate,q.lead,2.05,'through',dt,{direct:true})){q.returnTick=e.tick;q.expectedDirectReceiverId=e.player.id;e.pending(mate,e.player,'through');stage(e,'lane');return;}if(age(e)>3)terminal(e,false,'DEVOLUCIÓN TARDE','return-late');return;}
  if(q.customStage==='lane'){q.phase='Romper fuera de la trayectoria';hold(e,e.player,q.runLane,dt);face(e,e.player,q.lead,dt);if(e.ball.x>q.lineX-8)stage(e,'attack-return');else if(age(e)>3.2)terminal(e,false,'DEVOLUCIÓN CORTA','return-short');return;}
  ...
  if(receivedAfter(e,e.player,q.returnTick)&&(e.ball.x>q.lineX-4||e.player.x>q.lineX+10)){m.throughReceptions=Math.max(1,m.throughReceptions||0);terminal(e,true,'DESCARGA + RUPTURA','wall-run-complete');return;}if(age(e)>4.2)terminal(e,false,'DEVOLUCIÓN PERDIDA','return-missed');
}
...
function setupPress(e,rep){
  ...
  e.resetActor(e.player,680,FIELD.centerY+side*12,'ST');e.resetActor(cover,730,FIELD.centerY-side*110,'CAM');e.resetActor(wing,735,FIELD.centerY+side*160,'RW');e.resetActor(carrier,885,FIELD.centerY,'CB');e.resetActor(wide,820,FIELD.centerY+side*112,'RB');e.resetActor(pivot,755,FIELD.centerY-side*100,'CM');e.resetActor(keeper,FIELD.right-27,FIELD.centerY,'GK');
  ...
  Object.assign(e.trainingQualityV6,{side,screen:{x:770,y:FIELD.centerY-side*42},force:{x:795,y:FIELD.centerY-side*32},wideTarget:{x:wide.x,y:wide.y},wideTick:-1,possessionId:carrier.id});stage(e,'screen');e.repOrigin={px:e.player.x,py:e.player.y,bx:e.ball.x,by:e.ball.y};
}
function press(e,dt){
  ...
  if(q.customStage==='screen'){q.phase='1. Tapar pase interior';hold(e,wide,q.wideTarget,dt);hold(e,e.player,q.screen,dt);face(e,e.player,carrier,dt);anchor(e,carrier,q.wideTarget,dt,.25);if(dist(e.player,q.screen)<9&&facingDot(carrier,q.wideTarget)>.97)stage(e,'force-wide');return;}
  if(q.customStage==='force-wide'){q.phase='2. Orientar a banda';hold(e,e.player,q.force,dt);face(e,e.player,carrier,dt);hold(e,wide,q.wideTarget,dt);if(physicalKick(e,carrier,q.wideTarget,1.95,'press-wide',dt,{direct:true})){q.wideTick=e.tick;q.expectedDirectReceiverId=wide.id;e.pending(carrier,wide,'press-wide');stage(e,'jump');return;}return;}
  if(q.customStage==='jump'){q.phase='3. Saltar sobre el lateral';const intercept=e.projectedIntercept(wide);hold(e,wide,intercept,dt);hold(e,e.player,{x:intercept.x-46,y:intercept.y-q.side*16},dt);face(e,e.player,wide,dt);if(receivedAfter(e,wide,q.wideTick)){stage(e,'trap');e.flashTraining('ENCERRADO EN BANDA');return;}if(age(e)>3.2)terminal(e,false,'PASE PERDIDO','wide-missed');return;}
  q.phase='4. Forzar devolución';hold(e,e.player,{x:wide.x-34,y:wide.y-q.side*10},dt);face(e,e.player,wide,dt);if(dist(e.player,wide)<72&&physicalKick(e,wide,{x:carrier.x-8,y:carrier.y},1.9,'press-release',dt,{direct:true})){terminal(e,true,'PASE ATRÁS FORZADO','forced-back');return;}if(age(e)>3.2)terminal(e,false,'PRESIÓN TARDE','late');
}
