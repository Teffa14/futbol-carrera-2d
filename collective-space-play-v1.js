...
function isPresser(p,pressers){return Array.isArray(pressers)&&pressers.some(x=>(typeof x==='string'?x:x?.id)===p.id);}
function keepOnside(engine,p,target){if(roleFamily(p.role)!=='FWD'&&p.role!=='CAM')return target;const dir=attackDir(p.team),limit=onsideLimit(engine,p.team);return{...target,x:dir>0?Math.min(target.x,limit-7):Math.max(target.x,limit+7)};}
...
export function collectiveShapeTarget(engine,p,base,possession){
  ...
  const target=ours?keepOnside(engine,p,{x,y}):{x,y};return clampTarget(p,target);
}
...
export function postPassRunTarget(engine,p,receiver,kind='pass'){
  ...
  const x=receiver.x+dir*(forward+(pace-60)*.45),y=receiver.y+side*(kind==='wall'?72:52);return clampTarget(p,{x,y});
}
...
MatchEngine.prototype.executeKick=function collectiveRunAfterPass(p,contactNormal){
  ...
  const receiver=intent.receiverId?this.playerById(intent.receiverId):null;if(receiver&&p.role!=='GK'&&p.role!=='CB'){const target=postPassRunTarget(this,p,receiver,intent.passKind||intent.trainingKind||'pass');if(target)p.postPassRun={...target,fromTick:this.tick,untilTick:this.tick+58,receiverId:receiver.id,kind:intent.passKind||intent.trainingKind||'pass'};}
  return result;
};
...
MatchEngine.prototype.aiTarget=function collectiveSpaceFootball(p,pressers,actor,possession){
  const base=previousAiTarget.call(this,p,pressers,actor,possession);if(!p||p.role==='GK'||this.restart?.active)return base;const primaryPresser=possession!==null&&possession!==p.team&&isPresser(p,pressers);
  ...
  const collective=collectiveShapeTarget(this,p,base,possession);if(possession===p.team){const space=bestAttackingSpace(this,p,actor),w=roleWeight(p),raw={x:lerp(collective.x,space.x,w),y:lerp(collective.y,space.y,w)},target=keepOnside(this,p,raw);p.spaceRun={kind:'occupy-space',x:target.x,y:target.y,untilTick:this.tick+8};return clampTarget(p,target);}
...
export const __collectiveSpacePlayV1={collectiveShapeTarget,bestAttackingSpace,postPassRunTarget,crossTrajectoryTarget,spaceArrivalAdvantage,isPresser};