...
export function callSignalFor(option,p){
  const kind=option?.kind||'support',dir=attackDir(p.team),lead=kind==='through'||kind==='lob-through'||kind==='progressive',ahead=dir*(Number(option?.aim?.x??p.x)-p.x)>22;
  if(lead&&ahead)return{label:'¡AL ESPACIO!',kind:'ahead'};if(kind==='cutback'||kind==='cross')return{label:'¡ACÁ!',kind:'box'};return{label:'¡DÁMELA!',kind:'feet'};
}

export function bestCallForPass(engine,p,actor){
  if(!roleCanCall(p)||!actor||actor.team!==p.team||actor.id===p.id)return null;const option=chemistryAdjustedPassOptions(engine,actor).find(o=>o.player.id===p.id);if(!option||option.adjustedScore<.26||option.open<12)return null;
  const dir=attackDir(p.team),forward=dir*((option.aim?.x??p.x)-actor.x),spaceAhead=dir*((option.aim?.x??p.x)-p.x),progressive=['progressive','through','lob-through','cross','cutback'].includes(option.kind),score=option.adjustedScore+(progressive?.10:0)+clamp(forward/260,-.15,.18)+clamp(spaceAhead/130,-.08,.14);
  if(score<.38)return null;const signal=callSignalFor(option,p);return{option,score,signal,target:{x:keepOnside(engine,p,option.aim.x),y:clamp(option.aim.y,FIELD.top+p.r,FIELD.bottom-p.r)}};
}

function updateCalls(engine){
  ...
  const candidates=[];for(const p of engine.players){if(p.team!==possession||p.id===actor.id||!roleCanCall(p))continue;const call=bestCallForPass(engine,p,actor);if(call)candidates.push({p,...call});}
  candidates.sort((a,b)=>b.score-a.score);const allowed=new Set(candidates.slice(0,3).map(c=>c.p.id));
  for(const p of engine.players){if(p.team!==possession||!roleCanCall(p))continue;if(!allowed.has(p.id)){if(p.callForPass&&p.callForPass.untilTick<engine.tick+4)p.callForPass=null;continue;}const c=candidates.find(x=>x.p.id===p.id);const existing=p.callForPass;if(existing&&existing.untilTick>=engine.tick&&existing.fromId===actor.id)continue;const deep=c.signal.kind==='ahead',pace=Number(p.data?.pace??70),stamina=Number(p.data?.stamina??p.data?.physical??70);p.callForPass={fromId:actor.id,targetX:c.target.x,targetY:c.target.y,kind:c.signal.kind,label:c.signal.label,score:c.score,optionKind:c.option.kind,createdTick:engine.tick,untilTick:engine.tick+(deep?30:22)};p.action=c.signal.label;p.actionTimer=.48;if(deep&&pace>=58&&stamina>=52){p.burstTimer=Math.max(p.burstTimer||0,.24);p.burstX=attackDir(p.team);p.burstY=Math.sign(c.target.y-p.y)||0;}}
}
...
MatchEngine.prototype.step=function offBallCommunicationStep(dt){const out=previousStep.call(this,dt);if(!this.finished&&!this.restart?.active&&this.tick%14===0)updateCalls(this);for(const p of this.players)if(p.callForPass&&p.callForPass.untilTick<this.tick)p.callForPass=null;return out;};

export const __offballCommunicationV2={bestCallForPass,callSignalFor,updateCalls};
